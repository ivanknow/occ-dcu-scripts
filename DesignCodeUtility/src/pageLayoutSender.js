...
function transferStackInstanceChanges(sourceRegions, destinationRegions) {
  ...
  for (var index in sourceStackInstances) {
    ...
    const repositoryId = destinationStackInstances[index].repositoryId

    // Make sure we don't do too many requests at the same time.
    promises.push(updateStackSource("Less", repositoryId, sourceCode.less).then(() =>
      updateStackSource("LessVars", repositoryId, sourceCode.lessVars)).then(() =>
      updateStackSource("SourceCode", repositoryId, sourceCode.template)))
  }
 ...
 */
function updateStackSource(endpoint, repositoryId, source) {

  return endPointTransceiver[`updateStack${endpoint}`]([repositoryId], request().withBody({source}))
}
