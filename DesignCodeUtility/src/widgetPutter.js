  ...
  return Promise.each(elementInstances, elementInstance => {
      ...
      if (template.includes(getElementTagRepoId(elementInstance.tag)) &&
        ...
        return endPointTransceiver.createFragmentInstance(
          [metadata.repositoryId, getBaseElementTag(elementInstance.tag)]).then(results => {
...
function prepareLayoutSource(path, elementInstances) {
  ...
  return elementInstances.filter(elementInstance => elementInstance.oldTag).reduce((templateText, elementInstance) =>
...
function createMatchingWidgetInstance(widgetMetadata, path, updateCache = true) {
  ...
  return endPointTransceiver.createWidgetInstance([], request().withBody(payload)).then(() => updateCache && cacheWidgetInstances())
}
